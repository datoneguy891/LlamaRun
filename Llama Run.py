...
RUNNING = [pygame.image.load(os.path.join("Assets/Llama", "Llama.png")),
           pygame.image.load(os.path.join("Assets/Llama", "Llama2.png"))]
...
class Llama:
    ...
    def update(self, userInput):
        ...
        if userInput[pygame.K_UP] and not self.llama_jump:
            self.llama_run = False
            self.llama_jump = True
        elif not (self.llama_jump or userInput[pygame.K_DOWN]):
            self.llama_run = True
            self.llama_jump = False
...
def menu(death_count):
    ...
    while run:
        ...
        SCREEN.blit(text, textRect)
        SCREEN.blit(RUNNING[0], (SCREEN_WIDTH // 2 - 20, SCREEN_HEIGHT // 2 -
                                 140))
        pygame.display.update()
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
